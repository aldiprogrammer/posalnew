@extends('layouts.admin', ['title' => 'Data Pegawai'])

@section('content')
    <div class="flex items-center justify-between mb-6">
        <p class="text-gray-600">Kelola data pegawai.</p>
            ...
            <table class="w-full text-sm">
                <thead class="bg-gray-50 border-b border-gray-100">
                    <tr>
                        ...
                        <th class="text-left px-6 py-3 font-medium text-gray-500">No WA</th>
                        <th class="text-left px-6 py-3 font-medium text-gray-500">Alamat</th>
                        <th class="text-left px-6 py-3 font-medium text-gray-500">Jenis Kelamin</th>
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse ($pegawais as $pegawai)
                        <tr class="hover:bg-gray-50 transition-colors">
                            <td class="px-6 py-4 text-gray-500">{{ $pegawais->firstItem() + $loop->index }}</td>
                            <td class="px-6 py-4 font-medium text-gray-800">{{ $pegawai->nama }}</td>
                            <td class="px-6 py-4 text-gray-600">{{ $pegawai->jabatan }}</td>
                            <td class="px-6 py-4 text-gray-600">{{ $pegawai->no_wa ?? '-' }}</td>
                            <td class="px-6 py-4 text-gray-600 max-w-[200px] truncate" title="{{ $pegawai->alamat }}">{{ $pegawai->alamat ?? '-' }}</td>
                            <td class="px-6 py-4">
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $pegawai->jenis_kelamin === 'Laki-laki' ? 'bg-blue-100 text-blue-700' : 'bg-pink-100 text-pink-700' }}">
                                    {{ $pegawai->jenis_kelamin }}
                                </span>
                            </td>
                            <td class="px-6 py-4 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    <button onclick="openEditModal({{ $pegawai->toJson() }})" class="text-gray-400 hover:text-orange-600 transition-colors" title="Edit">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                                    </button>
                                    <form action="{{ route('admin.pegawai.destroy', $pegawai) }}" method="POST" onsubmit="return confirm('Yakin hapus pegawai ini?')">
                                        @csrf
                                        @method('DELETE')
                        ...
                        <div>
                            ...
                            <div class="flex gap-4">
                                <label class="flex items-center gap-2 cursor-pointer">
                                    <input type="radio" name="jenis_kelamin" value="Laki-laki" {{ old('jenis_kelamin') === 'Laki-laki' ? 'checked' : '' }} required class="text-orange-600 focus:ring-orange-500">
                                    <span class="text-sm text-gray-700">Laki-laki</span>
                                </label>
                                <label class="flex items-center gap-2 cursor-pointer">
                                    <input type="radio" name="jenis_kelamin" value="Perempuan" {{ old('jenis_kelamin') === 'Perempuan' ? 'checked' : '' }} required class="text-orange-600 focus:ring-orange-500">
    ...
    <div id="modal-edit" class="fixed inset-0 z-50 hidden">
        ...
        <div class="flex items-center justify-center min-h-screen p-4">
            <div class="bg-white rounded-xl shadow-xl w-full max-w-lg relative">
                <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100">
                    <h3 class="text-lg font-semibold text-gray-800">Edit Pegawai</h3>
                ...
                <form id="form-edit" method="POST">
                    ...
                    <div class="px-6 py-4 space-y-4">
                        ...
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">No. WhatsApp</label>
                            <input type="text" id="edit-no_wa" name="no_wa"
                                   class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-orange-500 focus:border-orange-500 outline-none transition-colors"
                                   placeholder="08xxxxxxxxxx">
                        </div>
                        <div>
                        ...
                        <div>
                            ...
                            <div class="flex gap-4">
                                <label class="flex items-center gap-2 cursor-pointer">
                                    <input type="radio" id="edit-jk-l" name="jenis_kelamin" value="Laki-laki" required class="text-orange-600 focus:ring-orange-500">
                                    <span class="text-sm text-gray-700">Laki-laki</span>
                                </label>
                                <label class="flex items-center gap-2 cursor-pointer">
                                    <input type="radio" id="edit-jk-p" name="jenis_kelamin" value="Perempuan" required class="text-orange-600 focus:ring-orange-500">
                                    <span class="text-sm text-gray-700">Perempuan</span>
                                </label>
                            </div>
                        </div>
...
<script>
    ...
    function openEditModal(pegawai) {
        ...
        form.action = '/admin/pegawai/' + pegawai.id;

        document.getElementById('edit-nama').value = pegawai.nama || '';
        document.getElementById('edit-jabatan').value = pegawai.jabatan || '';
        document.getElementById('edit-no_wa').value = pegawai.no_wa || '';
        document.getElementById('edit-alamat').value = pegawai.alamat || '';

        const jkL = document.getElementById('edit-jk-l');
        const jkP = document.getElementById('edit-jk-p');
        jkL.checked = pegawai.jenis_kelamin === 'Laki-laki';
        jkP.checked = pegawai.jenis_kelamin === 'Perempuan';
