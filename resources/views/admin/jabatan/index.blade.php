...
@section('content')
    <div class="flex items-center justify-between mb-6">
        <p class="text-gray-600">Kelola data jabatan.</p>
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse ($jabatans as $jabatan)
                        <tr class="hover:bg-gray-50 transition-colors">
                            <td class="px-6 py-4 text-gray-500">{{ $jabatans->firstItem() + $loop->index }}</td>
                            <td class="px-6 py-4 font-medium text-gray-800">{{ $jabatan->nama }}</td>
                            <td class="px-6 py-4 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    <button onclick='openEditModal(@json($jabatan))' class="text-gray-400 hover:text-orange-600 transition-colors" title="Edit">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                                    </button>
                                    <form action="{{ route('admin.jabatan.destroy', $jabatan) }}" method="POST" onsubmit="return confirm('Yakin hapus jabatan ini?')">
                                        @csrf
                                        @method('DELETE')
        ...
        <div class="flex items-center justify-center min-h-screen p-4">
            <div class="bg-white rounded-xl shadow-xl w-full max-w-md relative">
                <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100">
                    <h3 class="text-lg font-semibold text-gray-800">Tambah Jabatan</h3>
    ...
    <div id="modal-edit" class="fixed inset-0 z-50 hidden">
        ...
        <div class="flex items-center justify-center min-h-screen p-4">
            <div class="bg-white rounded-xl shadow-xl w-full max-w-md relative">
                <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100">
                    <h3 class="text-lg font-semibold text-gray-800">Edit Jabatan</h3>
...
<script>
    ...
    function openEditModal(jabatan) {
        ...
        form.action = '/admin/jabatan/' + jabatan.id;

        document.getElementById('edit-nama').value = jabatan.nama || '';
